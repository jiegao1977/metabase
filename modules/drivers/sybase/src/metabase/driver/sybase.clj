(ns metabase.driver.sybase
  ...
  (:import [java.sql Time DatabaseMetaData PreparedStatement ResultSet Connection Time Types]
...
(driver/register! :sybase, :parent #{:sql-jdbc ::legacy/use-legacy-classes-for-read-and-set})


; need remove from the source code.
...
;;Working to do the start  of month. '2020-09-01'
...
;repalce date-interval? not sure - question?
;(defmethod sql.qp/add-interval-honeysql-form :sybase
...
;jiegao unix timestamp
(defmethod sql.qp/unix-timestamp->honeysql [:sybase :seconds]
  [_ _ expr]
...
(defmethod sql.qp/unix-timestamp->honeysql [:sybase :milliseconds]
  [_ _ expr]
  (recur (hx// expr 1000) :sybase :seconds))
...
;(defmethod driver.common/current-db-time-date-formatters :sybase
;  [_]
;  (driver.common/create-db-time-formatters "yyyy-MM-dd'T'HH:mm:ss.SSSSSSSSZ"))
;
(defmethod driver.common/current-db-time-native-query :sybase
  [_]
  "select CONVERT(nvarchar(30), getdate(), 112)")

(defmethod driver/current-db-time :sybase
  [& args]
  (apply driver.common/current-db-time args))

(defmethod sql.qp/current-datetime-honeysql-form :sybase [_] :%getdate)

(defmethod sql-jdbc.sync/excluded-schemas :sybase
  [_]
...
;(defmethod sql.qp/->honeysql [SybaseDriver Boolean]
;  [_ bool]
;  (if bool 1 0))
;
;(defmethod sql.qp/->honeysql [SybaseDriver Time]
;  [_ time-value]
;  (hx/->time time-value))
;
